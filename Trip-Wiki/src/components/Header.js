export default function Header({
  ...
  this.template = () => {
    ...
    let temp = `<div class="title"><a href="/">✈Trip Wiki</a></div>`;
    ...
    <div class="search">
    <input type="text" placeholder="Search" id="search" autocomplete="off" value=${searchWord} ></div>
    </div>`;
    }
  ...
  this.render = () => {
    ...
    if (!this.state.currentPage.includes("/city/")) {
      ...
      $searchInput.addEventListener("keydown", (event) => {
        if (event.key === "Enter") {
          this.handleSearch($searchInput.event.value);
        }
      });
    }
  };
